...
# ... (Devolver un JSON y realizar una consulta de agregación sobre la entidad Data) ...

from django.db.models import Avg
from django.utils import timezone
from datetime import timedelta

def api_promedio_estaciones(request):
    """
    Reto Capa de Datos: Consulta interesante que calcula el 
    promedio de los valores recibidos por cada estación 
    en la última hora.
    """
    hace_una_hora = timezone.now() - timedelta(hours=1)
    
    # Agrupamos por estación y calculamos el promedio del campo 'value'
    query = Data.objects.filter(time__gte=hace_una_hora).values('station_id').annotate(promedio=Avg('value'))
    
    # Devolvemos el resultado en formato JSON como pide el reto
