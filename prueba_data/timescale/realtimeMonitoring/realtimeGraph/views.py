...
def top_stations(request):
    measurement_name = request.GET.get('measurement', 'temperatura')
    start = request.GET.get('start', '2021-07-01')
    end = request.GET.get('end', '2021-07-31')
    top_n = int(request.GET.get('top', 5))

    start_ts = int(datetime.strptime(start, '%Y-%m-%d').timestamp() * 1_000_000)
    end_ts = int(datetime.strptime(end, '%Y-%m-%d').timestamp() * 1_000_000)
    ...
    data = [
        {
            ...
            "min_value": round(r['min_value'] or 0, 2),
            "max_value": round(r['max_value'] or 0, 2),
